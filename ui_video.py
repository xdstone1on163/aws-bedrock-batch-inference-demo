...
from job_handlers import (
    preview_files, validate_configuration, start_video_batch_job,
    refresh_job_status, get_results, validate_video_single_inference
)


def create_video_batch_interface(job_arn_state, results_ready_state):
    """创建视频批处理界面"""
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Group():
                ...
                use_existing_jsonl = gr.Checkbox(
                    label="使用已有的JSONL文件（跳过视频处理）",
                    value=False,
                    info="如果您已经准备好了视频批处理的JSONL文件，勾选此项"
                )
                ...
                # 两种模式的输入配置
                with gr.Group() as raw_videos_group:
                    gr.Markdown("#### 🎬 原始视频模式")
                    s3_input_prefix = gr.Textbox(
                        label="视频S3路径前缀",
                        value="batch-demo-videos-101-items/",
                        placeholder="例如: videos/folder（系统会自动处理末尾的'/'）",
                        info="留空表示bucket根目录"
                    )
                ...
                with gr.Group(visible=False) as jsonl_file_group:
                    ...
                    jsonl_file_s3_uri = gr.Textbox(
                        label="JSONL文件S3 URI *",
                        placeholder="例如: s3://my-bucket/input/video-batch.jsonl",
            ...
            with gr.Group():
                ...
                with gr.Row():
                    ...
                    output_prefix = gr.Textbox(
                        label="输出路径前缀",
                        value="input/batch-video-output/",
                        placeholder="例如: output/videos（系统会自动处理末尾的'/'）",
                        info="留空表示bucket根目录"
                    )
            ...
            with gr.Group():
                ...
                user_prompt_input = gr.Textbox(
                    label="User Prompt *",
                    placeholder="例如: 请详细描述这个视频的内容",
                    lines=5,
                    info="用户提示词，描述对视频的具体要求"
                )
                
                with gr.Row():
                    model_dropdown = gr.Dropdown(
                        choices=list(VIDEO_MODELS.keys()),
                        value="Nova Pro (Video)",
                        label="选择模型 *",
                        info="支持视频理解的Nova模型"
                    )
                ...
                # 操作按钮
                with gr.Row():
                    preview_btn = gr.Button("🔍 预览视频", variant="secondary")
        ...
        with gr.Column(scale=3):
            ...
            with gr.Group():
                preview_message = gr.Markdown("点击'预览视频'查看S3视频列表")
    ...
    with gr.Accordion("📖 使用说明", open=False):
        ...
        **⚠️ 视频文件大小限制：**
        - **单个视频文件建议不超过20MB**
        - Base64编码后的payload大小限制为25MB
        - 超过限制的视频将被自动跳过
        - 建议使用视频压缩工具减小文件大小
        
        **输入视频准备：**
        - 请提前将待处理的视频上传到S3 bucket
        - 支持的视频格式：MP4, MOV, AVI, MKV, WEBM, FLV
        - 系统会自动从指定路径读取所有视频文件
        
        **Prompt说明：**
        - **System Prompt**（可选）：设定模型的角色和行为方式
        - **User Prompt**（必填）：描述您对视频的具体分析要求
        ...
        **模型参数：**
        ...
        3. **预览视频**: 可选，查看S3中待处理的视频列表
        4. **输入Prompt**: 设置System Prompt和User Prompt
        5. **选择模型和Role**: 选择支持视频的Nova模型和IAM角色
        6. **验证权限**: 可选，确保配置正确
        7. **开始处理**: 提交批处理任务
        8. **监控和获取结果**: 刷新状态并获取结果
        """)
    ...
    use_existing_jsonl.change(
        ...
        outputs=[raw_videos_group, jsonl_file_group, preview_btn]
    )
    ...
    validate_inference_btn.click(
        fn=validate_video_single_inference,
        inputs=[
            use_existing_jsonl, input_bucket, s3_input_prefix, jsonl_file_s3_uri,
